...
document.addEventListener("DOMContentLoaded", function () {
   ...
   function fetchNodeTypesAndVisualizeGraph(data) {
       // If backend failed, show error and stop.
       if (!data || data.success === false) {
           console.error("Backend returned error instead of graph data:", data && data.error);
           alert("Cypher query failed:\n" + (data && data.error ? data.error : "Unknown error"));
           return;
       }
       ...
       fetch(`/get_node_types?projectName=${encodeURIComponent(projectName)}`)
           .then(response => response.json())
           .then(nodeTypes => {
               gAllowedNodeLabels = nodeTypes.map(type => type.name);
   ...
   network.on('doubleClick', function (params) {
       ...
       .then(response => {
           ...
           if (!contentType.toLowerCase().includes("application/json")) {
               return response.text().then(text => {
                   console.error("Non-JSON response from /expand-node:", text);
       ...
       .then(data => {
           console.log("EXPAND-NODE response:", data);
           if (!data || data.success === false) {
               console.error("Expand-node failed:", data && data.error);
               alert("Expand failed:\n" + (data && data.error ? data.error : "Unknown error"));
               return;
           }
           console.log("data")
...
function fetchNodeTypes() {
   // resolve projectName from preferred globals/storage
   const projectName = window.projectName
       || window.currentProject
       || (typeof currentProject !== 'undefined' && currentProject)
       || localStorage.getItem('iv_project')
...
function submitNodeType(pNodeType, pName, pShape, pColor, pSize) {
   ...
   const createdBy = window.currentEmail
       ...
       || localStorage.getItem('iv_email')
       || sessionStorage.getItem('iv_email')
       || null;

   console.log('submitNodeType -> projectName, createdBy:', projectName, createdBy);    
